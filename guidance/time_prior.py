...
class TimePrioritizedScheduler:
    def __init__(
        self, cfg: GuideConfig, scheduler, device, num_train_timesteps=1000
    ) -> None:
        # Hyper Params
        self.device = device
        ...
        if self.time_sampling.startswith("stage"):
            ...
            for i in range(self.num_stage, 0, -1):
                self.intervals_per_stage.append(
                    (
                        self.min_step + timesteps_per_stage * (i - 1),
                        self.min_step + timesteps_per_stage * i,
                    ),
                )
    ...
    def get_timestep(self, batch_size, train_step, max_iteration):
        if self.time_sampling == "uniform":
            ...
            timestep = self.get_annealed_t_from_weights_cumsum(
                train_step, max_iteration
            )
            t = (
                torch.ones(
                    [
    ...
    def get_annealed_t_from_weights_cumsum(self, train_step, max_iteration):
        ...
        # Bi-Search
        t = self.num_timesteps - bisect.bisect_left(
            self.weights_cumsum, current_state
        )  # [0, ..., 1000]
    ...
    def draw_curves(self, save_dir):
        ...
        t_each_step = [
            self.get_timestep(1, i, len(train_steps)).item() for i in train_steps
        ]
        plt.figure()
